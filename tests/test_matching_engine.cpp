...
TEST_F(MatchingEngineTest, NoMatchWhenBookEmpty) {
  size_t n = engine.match(1, Side::BID, 1010, 100, fills);
  EXPECT_EQ(n, 0u);
}

TEST_F(MatchingEngineTest, NoMatchWhenPriceTooLow) {
  book.add_order(1, 100, 1015, Side::ASK);
  size_t n = engine.match(99, Side::BID, 1010, 100, fills);
  EXPECT_EQ(n, 0u);
}
...
TEST_F(MatchingEngineTest, FullFillSingleOrder) {
  ...
  size_t n = engine.match(99, Side::BID, 1010, 100, fills);
  ASSERT_EQ(n, 1u);
  EXPECT_EQ(fills[0].pass_order_id, 1u);
  EXPECT_EQ(fills[0].agg_order_id, 99u);
  EXPECT_EQ(fills[0].qty, 100u);
  EXPECT_EQ(fills[0].price, 1010u);
}

TEST_F(MatchingEngineTest, PartialFillSingleOrder) {
  book.add_order(1, 200, 1010, Side::ASK);
  size_t n = engine.match(99, Side::BID, 1010, 50, fills);
  ASSERT_EQ(n, 1u);
...
TEST_F(MatchingEngineTest, FillAcrossMultiplePriceLevels) {
  ...
  size_t n = engine.match(99, Side::BID, 1011, 100, fills);
  ASSERT_EQ(n, 2u);
  EXPECT_EQ(fills[0].price, 1010u);
  EXPECT_EQ(fills[1].price, 1011u);
}

TEST_F(MatchingEngineTest, AskMatchesBid) {
  book.add_order(1, 100, 1010, Side::BID);
  size_t n = engine.match(99, Side::ASK, 1010, 100, fills);
  ASSERT_EQ(n, 1u);
  EXPECT_EQ(fills[0].qty, 100u);
}